...
sqlconnection.query(sqlGetCalData, [ eventType ], function (error, events) {
    ...
    if (events){
        ...
        for (const event of events) {
            ...
            let busyStatus = 'FREE'
            let start = [ event.startDate.getFullYear(), event.startDate.getMonth(), event.startDate.getDate(), event.startDate.getHours(), event.startDate.getMinutes() ]
            let duration = { days: event.duration }
            let uid = event.uid
            let created = [ event.created.getFullYear(), event.created.getMonth(), event.created.getDate(), event.created.getHours(), event.created.getMinutes() ]
